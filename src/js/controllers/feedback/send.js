  ...
  $scope.$on('$ionicView.beforeEnter', function (event, data) {
    ...
    switch ($scope.score) {
      ...
      case 5:
        ...
        $scope.comment = gettextCatalog.getString("We're always looking for ways to improve the app.", {
          appName: appConfigService.nameCase
        }) + ' ' + gettextCatalog.getString('Is there anything we could do better?')
        break
      default:
        $scope.justFeedback = true
        $scope.comment = gettextCatalog.getString("We're always looking for ways to improve the app. How could we improve your experience?", {
          appName: appConfigService.nameCase
        })
        break
    }
