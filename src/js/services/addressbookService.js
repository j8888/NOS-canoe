...
angular.module('canoeApp.services').factory('addressbookService', function ($log, nanoService, storageService, lodash) {
  ...
  root.save = function (entry, oldAddress, cb) {
    if (!nanoService.isValidAccount(entry.address)) return cb('Not valid NOS account')
    storageService.getAddressbook(function (err, ab) {
      if (err) return cb(err)
      if (ab) ab = JSON.parse(ab)
      ab = ab || {}
      ...
      if ((entry.address !== oldAddress) && ab[entry.address]) return cb('Other entry with that NOS account already exists')
  ...
  root.add = function (entry, cb) {
    ...
    storageService.getAddressbook(function (err, ab) {
      ...
      ab = ab || {}
      if (lodash.isArray(ab)) ab = {} // No array
      if (ab[entry.address]) return cb('Other entry with that NOS account already exists')
      ab[entry.address] = entry
      storageService.setAddressbook(JSON.stringify(ab), function (err, ab) {
        if (err) return cb('Error adding new entry')
        root.list(function (err, ab) {
  ...
  root.remove = function (addr, cb) {
    if (!nanoService.isValidAccount(addr)) return cb('Not valid NOS account')
    storageService.getAddressbook(function (err, ab) {
      if (err) return cb(err)
      if (ab) ab = JSON.parse(ab)
      ab = ab || {}
